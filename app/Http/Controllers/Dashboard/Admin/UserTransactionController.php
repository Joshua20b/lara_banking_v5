...
class UserTransactionController extends Controller
{
    ...
    public function store(Request $request, $uuid)
    {
        ...
        switch ($request->type) {
            case 'CREDIT':
                switch ($request->account) {
                    case 'income_balance':
                        $user->income_balance = $user->income_balance + $amount;
                        $user->save();
                        break;
                    case 'loan_balance':
                        $user->loan_balance = $user->loan_balance + $amount;
                        $user->save();
                        break;
                ...
                break;
            case 'DEBIT':
                if ($request->amount == 'balance' && $amount > $balance) {
                    session()->flash('error', 'Transaction failed insufficient balance');
                    return redirect()->back();
                }
                if ($request->account == 'income_balance' && $amount > $user->income_balance) {
                    session()->flash('error', 'Transaction failed insufficient income balance');
                    return redirect()->back();
                }
                if ($request->account == 'loan_balance' && $amount > $user->loan_balance) {
                    session()->flash('error', 'Transaction failed insufficient loan balance');
                    return redirect()->back();
                }
        ...
        $transactionData['current_balance'] = $user->balance;
